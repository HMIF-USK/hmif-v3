...
const multiSelectVariants = cva('m-1 transition ease-in-out', {
  variants: {
    variant: {
      default: 'border-white/10 text-white bg-white hover:bg-white/80',
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className={cn('relative w-full', baseClassName)}>
          <Select
            ...
            disabled={disabled}
          >
            <SelectTrigger className="h-[48px] px-4 outline-none focus-visible:ring-transparent focus:ring-transparent focus:border-purple-500 border-2 border-border rounded-lg text-white">
        ...
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
          <PopoverTrigger asChild>
            <Button
              ...
              className="flex w-full p-1 rounded-lg border-2 border-border min-h-10 h-auto items-center justify-between bg-white hover:bg-white focus:border-purple-500"
              disabled={disabled}
            >
              {selectedValues.length > 0 ? (
                <div className="flex justify-between items-center w-full">
                  <div className="flex flex-wrap items-center">
                    ...
                    {!allSelected && selectedValues.length > maxCount && (
                      <Badge
                        className={cn(
                          'bg-transparent text-white border-white/10 hover:bg-transparent',
                          isAnimating ? 'animate-bounce' : '',
                          multiSelectVariants({ variant })
                        )}
                        style={{ animationDuration: `${animation}s` }}
                ...
                <div className="flex items-center justify-between w-full mx-auto">
                  <span className="text-base text-muted-foreground mx-3">{placeholder}</span>
          ...
          {animation > 0 && selectedValues.length > 0 && (
            <WandSparkles
              className={cn(
                'cursor-pointer my-2 text-white bg-background w-3 h-3',
