...
interface IArticleSectionProps {
  data?: IArticle;
  /** Artikel lain untuk section "baca juga"; dikirim pemanggil dari API. */
  related?: IArticle[];
}
const ArticleSection: React.FC<IArticleSectionProps> = ({ data, related = [] }) => {
  const navigation = useRouter();
  const nextAchievement = related.filter((item) => item.slug !== data?.slug).slice(0, 3);
            ...
            >
              ...
              <defs>
                ...
                >
                  ...
                  <stop offset="1" stopColor="#4A207D" />
                </linearGradient>
              </defs>
            </svg>
          </div>
